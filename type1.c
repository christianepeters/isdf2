 ...
 *
 *  compile: 
 *    gcc type1.c -o type1 -lm -lgmp -lmpfr -lmpfi
 * 
 */
...
void P_compute(int p)
{
  ...
  for(u = 0; u <= w; u++)
  {
    mpfi_set_ui(tmp0, (1.*k-u)*(n-k-(w-u))+u*(w-u));
    mpfi_div_ui(P[u][u], tmp0, k*(n-k)); 
  }

  /* transition: weight u -> weight u-1 */
  for(u = 1; u <= w; u++)
  {
    mpfi_set_ui(tmp0, u*(n-k-(w-u)));
    mpfi_div_ui(P[u][u-1], tmp0, k*(n-k)); 
  }
  
  /* transition: weight u -> weight u+1 */
  for(u = 0; u < w; u++)
  {
    mpfi_set_ui(tmp0, (k-u)*(w-u));
    mpfi_div_ui(P[u][u+1], tmp0, k*(n-k)); 
  }

  /* the (2p)_S state is placed in the last entry of the matrix P */
  /* transition: 2p+1 -> (2p)_S */
  mpfi_set_ui(tmp0, (2*p+1)*(n-k-(w-(2*p+1))));
  mpfi_div_ui(P[2*p+1][w+1], tmp0, k*(n-k)); 
  
  /* transition: 2p-1 -> (2p)_S */
  mpfi_set_ui(tmp0, (k-(2*p-1))*(w-(2*p-1)));
  mpfi_div_ui(P[2*p-1][w+1], tmp0, k*(n-k)); 

  /* transition: (2p)_F -> (2p)_S */
  mpfi_set_ui(tmp0, (k-2*p)*(n-k-(w-2*p)) + 2*p *(w-2*p));
  mpfi_div_ui(P[2*p][w+1], tmp0, k*(n-k)); 
...
void R_compute(int p)
{
  ...
  /* case |supp(c) intersects (X union Y)| = 2p */
  id_mat(Tmp, w+2);
...
//  for (w = 4; w<=50; w++)
  {
    ...
    /* compute c-th power of the w+1-dim submatrix of P */
    copy_mat(R, P, w+1);
    id_mat(Pc, w+1);
